...
def page_preview(admin_view: bool = False):
    ...
    with st.expander("🔍 Filter Data", expanded=False):
        ...
        with fc1:
            f_company = st.text_input("Company Name contains")
        with fc2:
            f_status  = st.selectbox("Status", ["All","In Process","Cancelled","Hold","Completed","Postponed"])
        with fc3:
        ...
        if admin_view:
            ...
            with fc4:
                ...
                f_mgr = st.selectbox("Manager", mgr_list)
        else:
            f_mgr = "All"
    ...
    if f_status != "All":
        filtered = filtered[filtered["Company Current Status"] == f_status]
    if f_batch != "All":
        filtered = filtered[filtered["Batch"] == f_batch]
    if f_mgr != "All":
        filtered = filtered[filtered["Submitted By"] == f_mgr]
    ...
    # ── Row-by-row preview with Edit button ──────────────────────────────────
    original_indices = filtered.index.tolist()

    for display_i, orig_i in enumerate(original_indices):
        row = filtered.loc[orig_i]
        with st.expander(f"📌 {row.get('Company Name','—')}  |  {row.get('Floated Date','—')}  |  {row.get('Company Current Status','—')}"):
