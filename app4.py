...
def page_preview(admin_view=False):
    ...
    # ── DISPLAY: scrollable full dataframe for all columns ────────────────────
    st.markdown("#### 📊 Data Table")

    # Columns to show in the compact action table (identity columns)
    ACTION_COLS = ["Company Name", "Floated Date", "Batch", "Opportunity Type",
                   "Core/Non-Core", "CTC (LPA)", "Company Current Status", "Submitted By"]
    show_cols = [c for c in ACTION_COLS if c in display_df.columns]

    # Full data view (scrollable, read-only native table)
    ...
    # ── ACTION TABLE: each row = key info + Edit + Delete in one line ─────────
    st.markdown("---")
    st.markdown("#### ✏️ Edit / Delete")
    st.caption("Same row order as table above.")

    # widths: key cols share 8 parts, Edit takes 0.8, Delete takes 0.8
    col_widths = [2, 1.3, 1.2, 1.5, 1, 1.3, 1.5, 1.3, 0.8, 0.8]
    labels     = show_cols + ["", ""]

    # Header row
    header_cols = st.columns(col_widths)
    for hc, lbl in zip(header_cols, labels):
        hc.markdown(f"<div style='font-size:0.75rem;font-weight:700;color:#5a7fa5;padding-bottom:4px'>{lbl}</div>",
                    unsafe_allow_html=True)

    # Divider line
    st.markdown("<hr style='margin:2px 0 6px 0;border-color:#dde4ef'>", unsafe_allow_html=True)
    ...
    for _, row in flt.iterrows():
        ...
        for ci, col_name in enumerate(show_cols):
            ...
            row_cols[ci].markdown(
                f"<div style='font-size:0.82rem;padding:2px 0'>{val}</div>",
                unsafe_allow_html=True)
        # Edit button
        with row_cols[len(show_cols)]:
            if st.button("✏️ Edit", key=f"edit_{orig_i}", use_container_width=True, type="primary"):
                ...
                st.rerun()
        # Delete button
        with row_cols[len(show_cols)+1]:
            if st.button("🗑️ Del", key=f"del_{orig_i}", use_container_width=True):
                all_data = load_data()
                all_data.pop(orig_i)
                save_data(all_data)
